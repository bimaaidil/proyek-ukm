...
function UkmEdit() {
    ...
    useEffect(() => {
        const fetchUkmData = async () => {
            try {
                const response = await api.get(`/api/ukm/${id}`);
                const ukmData = response.data;
                const combinedData = { ...ukmData.user, ...ukmData };

                if (combinedData.tanggal_lahir) { combinedData.tanggal_lahir = new Date(combinedData.tanggal_lahir).toISOString().split('T')[0]; }
                if (combinedData.tanggal_nib) { combinedData.tanggal_nib = new Date(combinedData.tanggal_nib).toISOString().split('T')[0]; }
    ...
    return (
        <Container className="my-5">
            <Row className="justify-content-md-center">
                <Col md={10}>
                    <Card>
                        <Card.Header as="h4">Edit Data Pelaku & Brand Usaha</Card.Header>
                        <Card.Body>
